...
#include "SpecialParserRegistry.hpp"


//--------------------------------------------------------------------
parser::ParseResult parser::DefaultCommandParser::parser(std::u8string raw_string, bool shared_strict_flag /*= true*/, bool special_strict_flag /*= false*/) noexcept
{
    details::FieldParser fp;
    std::pair<std::optional<details::CommandFields>, details::ErrorMsgs> parser_result = fp.field_parser(raw_string);
    ...
    catch (details::ParserException const& pe)
    {
    ...
    if (parser_result.first.has_value())
        ...
        return ParseResult{ details::CommandFields{}, error_messages, warning_messages };
...
//--------------------------------------------------------------------
parser::details::WarnMsgs parser::DefaultCommandParser::check(std::pair<std::optional<details::CommandFields>, details::ErrorMsgs> parser_result, bool shared_strict_flag /*= true*/, bool special_strict_flag /*= false*/)
{
    // 如果字段解析有问题，直接抛
    if (!parser_result.second.empty())
        throw details::ParserException{ parser_result.second };

    // 再看一眼解析出来的字段是否存在问题
    auto parser_result_fields = parser_result.first;
    if (!parser_result_fields.has_value())
        throw details::ParserException{ std::vector<std::u8string>{ u8"field parser error" } };

    // 检查公有字段是否都存在
    // shared_strict_falg 表示是否将 Warning 算作出错，默认为 true
    std::tuple<details::CommandFields, details::ErrorMsgs, details::WarnMsgs> shared_check_result = details::SharedParser::shared_check(*parser_result_fields);
    ...
    if (!shared_error_msgs.empty() || shared_strict_flag && !shared_warn_msgs.empty())
    {
        ...
        throw details::ParserException{ shared_error_msgs };
    }

    // 通过 Command，找到对应的私有字段校验函数
    auto it1 = shared_command_fields.find(u8"Command");
    if (it1 == shared_command_fields.end())
        throw details::ParserException{ {u8"not find command field"} };

    // compile
    auto command_type = it1->second;
    auto& parsers = details::special_parsers();

    auto it2 = parsers.find(command_type);
    if (it2 == parsers.end())
        throw details::ParserException{ {u8"don't have " + command_type + u8" type"} };
    ...
    std::tuple<details::CommandFields, details::ErrorMsgs, details::WarnMsgs> special_check_result = special_check_func(command_fields_copy);
    ...
    if (!special_error_msgs.empty() || special_strict_flag && !special_warn_msgs.empty())
    {
        ...
        throw details::ParserException{ special_error_msgs };
    }
    ...
    return details::WarnMsgs{ warning_messages };
}
